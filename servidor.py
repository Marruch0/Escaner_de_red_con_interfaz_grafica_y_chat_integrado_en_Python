...
def procesar_comando(cmd, partes, socket, username, sockets, usuarios, roles):
    ...
    # Comando /log (para registrar eventos desde el cliente)
    elif cmd == "/log" and len(partes) >= 2:
        # Construir el mensaje completo
        mensaje_log = ' '.join(partes[1:])
        # Guardar directamente en el log sin necesidad de responder al cliente
        guardar_log("SISTEMA", mensaje_log)
        return True  # Para que no se intente enviar respuesta innecesaria
    
    # Comando desconocido
    else:
        socket.sendall("ERROR: Comando desconocido. Usa /help para er la lista de comandos\n".encode())
