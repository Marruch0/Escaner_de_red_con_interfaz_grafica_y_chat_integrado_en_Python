...
ICONS_MAP = {
    ...
    "proxmox": "https://cdn-icons-png.flaticon.com/512/4380/4380600.png"  # Icono de cloud-server para Proxmox
}
...
def obtener_tipo_dispositivo(mac):
    ...
    oui_map = {
        ...
        '6045': 'D-Link',
        'fc15': 'Mediatek',
        '001f29': 'Proxmox',  # Añadido Proxmox (este es un OUI ilustrativo, ajustar según corresponda)
        '525400': 'Proxmox'   # Proxmox suele usar este prefijo en algunas configuraciones
    }
...
def obtener_ruta_icono(tipo_dispositivo):
    ...
    elif "proxmox" in tipo:
        return os.path.join(ICONS_DIR, "proxmox.png")
...
class PortScannerGUI:
    def __init__(self, parent_frame):
        ...
        self.var_analyze_banners = IntVar(value=1)  # Nuevo: Analizar banners
        self.var_detect_os = IntVar(value=1)        # Nuevo: Detectar sistema operativo
        ...
        # Nuevo: Checkbox para analizar banners
        self.check_analyze_banners = CTkCheckBox(self.panel_opciones, text="Analizar banners", 
                                                 variable=self.var_analyze_banners,
                                                 text_color=self.color_texto_claro,
                                                 fg_color=self.color_boton, hover_color=self.color_hover)
        self.check_analyze_banners.pack(pady=5, anchor="w")

        # Nuevo: Checkbox para detectar sistema operativo
        self.check_detect_os = CTkCheckBox(self.panel_opciones, text="Detectar sistema operativo", 
                                           variable=self.var_detect_os,
                                           text_color=self.color_texto_claro,
                                           fg_color=self.color_boton, hover_color=self.color_hover)
        self.check_detect_os.pack(pady=5, anchor="w")
        
        # Velocidad de escaneo
        self.panel_tiempo = CTkFrame(self.panel_config, fg_color=self.color_panel)
        self.panel_tiempo.pack(fill="x", padx=10, pady=5)
        
        self.label_timing = CTkLabel(self.panel_tiempo, text="Velocidad de Escaneo:", 
                                   anchor="w", text_color=self.color_texto_claro)
    ...
    def mostrar_resultados(self, results, os_detected, total_time):
        """Muestra los resultados del escaneo en la interfaz sin colores"""
        # Actualizar pestaña de puertos
        self.area_puertos.configure(state='normal')
        self.area_puertos.delete("1.0", END)
        
        analyze_banners = bool(self.var_analyze_banners.get())  # Verificar si analizar banners está activado
        detect_os = bool(self.var_detect_os.get())              # Verificar si detectar sistema operativo está activado
        ...
        if results:
            ...
            for port, status, service, banner in results:
                # Formatear texto del puerto
                texto_puerto = f"Puerto {port}: {status} ({service})\n"
                self.area_puertos.insert(END, texto_puerto)
                
                # Mostrar banners si está activado
                if analyze_banners and banner != "N/A":
                    texto_banner = f"  Banner: {banner}\n"
                    self.area_puertos.insert(END, texto_banner)
        
            # Mostrar sistema operativo si está activado
            if detect_os:
                texto_os = f"\nSistema operativo detectado: {os_detected}\n"
            
            self.area_puertos.insert(END, texto_os)
            
            # Tiempo total del escaneo
            texto_tiempo = f"Tiempo total de escaneo: {total_time:.2f} segundos"
            self.area_puertos.insert(END, texto_tiempo)
        else:
    ...
    def iniciar_escaneo(self):
        ...
        analyze_banners = bool(self.var_analyze_banners.get())  # Nuevo
        detect_os = bool(self.var_detect_os.get())              # Nuevo
        ...
        def run_scan():
            try:
                ...
                # Detectar sistema operativo si está habilitado
...
# Reemplaza toda la sección del código de zoom con esta implementación corregida:

# Variable global para el nivel de zoom
nivel_zoom = 1.0
label_zoom_nivel = None

def ajustar_zoom(cambio):
    """Ajusta el zoom de la interfaz según el incremento proporcionado."""
    global nivel_zoom, label_zoom_nivel, ventana
    if label_zoom_nivel is None:
        print("Error: label_zoom_nivel no está inicializado")
        return
    nivel_zoom += cambio
    nivel_zoom = max(0.5, min(2.0, nivel_zoom))  # Limitar entre 0.5x y 2.0x
    
    # Aplicar escala a los widgets principales
    set_widget_scaling(nivel_zoom)
    
    # Actualizar etiqueta de nivel de zoom
    label_zoom_nivel.configure(text=f"Zoom: {int(nivel_zoom * 100)}%")
    
    # Redimensionar ventana si es necesario
    nuevo_ancho = int(ancho * nivel_zoom)
    nuevo_alto = int(alto * nivel_zoom)
...
def es_mensaje_admin(mensaje):
    ...
    patrones = [
        ...
        "Ejecutando:", "creado con éxito", "ya existe", "no existe", 
        "Lista de usuarios baneados", "añadido", "modificado", "actualizado"
    ]
    ...
    while True:
        try:
            ...
            if not mensaje:
                ...
                area_chat.insert(END, "\n\nConexión cerrada por el servidor. La aplicación se cerrará en 5 segundos...\n")
                area_chat.see(END)
            ...
            if area_admin and es_mensaje_admin(mensaje):
                ...
                # Verificar si es una continuación de mensaje anterior
                if mensaje.startswith("+") or mensaje.startswith("Usuario "):
                    # Añadir al texto existente
                    area_admin.insert(END, mensaje + "\n")
                else:
                    # Es un nuevo mensaje administrativo, borrar todo y mostrar solo este
                    area_admin.delete("1.0", END)
                    area_admin.insert(END, mensaje + "\n")
                
                area_admin.see(END)
                area_admin.configure(state='disabled')
                continue  # Importante: continuar para evitar mostrar en el chat
            ...
            area_chat.configure(state='normal')
            area_chat.insert(END, "\n\nError de conexión con el servidor. La aplicación se cerrará en 5 segundos...\n")
            area_chat.see(END)
            area_chat.configure(state='disabled')
            ventana.after(5000, lambda: os._exit(0))
            return
...
def ventana_login():
    ...
    def iniciar_chat():
        ...
        try:
            ...
            socket_cliente = ssl._create_unverified_context().wrap_socket(socket_cliente, server_hostname=host)
...
def crear_panel_admin(frame, socket, area_chat):
    ...
    def enviar_comando(comando):
        try:
            ...
            socket.getpeername()  # Generará error si el socket está cerrado
            
            # Limpiar completamente el área de resultados
            area_resultado.configure(state='normal')
            area_resultado.delete("1.0", END)
            ...
            area_resultado.configure(state='normal')
            area_resultado.delete("1.0", END)
            area_resultado.insert(END, f"Error al enviar comando: {e}\nLa conexión puede haberse perdido.")
            area_resultado.configure(state='disabled')
    ...
    boton_usuarios = CTkButton(fila4, text="Ver Usuarios", 
                         command=lambda: (
                             area_resultado.configure(state='normal'),
                             area_resultado.delete("1.0", END),
                             area_resultado.insert(END, "Obteniendo lista de usuarios...\n"),
                             area_resultado.configure(state='disabled'),
                             socket.sendall("/userlist".encode())
                         ), 
                         fg_color=color_boton, hover_color=color_hover, width=100)
    ...
    boton_baneados = CTkButton(fila4, text="Ver Baneados", 
                         command=lambda: (
                             area_resultado.configure(state='normal'),
                             area_resultado.delete("1.0", END),
                             area_resultado.insert(END, "Obteniendo lista de usuarios baneados...\n"),
                             area_resultado.configure(state='disabled'),
                             socket.sendall("/banlist".encode())
                         ), 
                         fg_color=color_boton, hover_color=color_hover, width=100)
...
def iniciar_cliente(username, socket, role):
    ...
    frame_cuenta.pack(side="bottom", fill="x", pady=(5, 10), padx=10)  # Reducido el padding inferior
    
    # Añadir un panel para ajustar el zoom en el menú lateral (MOVIDO AQUÍ)
    frame_zoom = CTkFrame(menu, fg_color="transparent")
    frame_zoom.pack(side="bottom", fill="x", pady=(5, 10), padx=10)  # Antes del usuario
    
    # Etiqueta para el zoom
    label_zoom = CTkLabel(frame_zoom, text="Ajustar zoom", text_color=color_texto_claro, font=("Arial", 12))
    label_zoom.pack(anchor="w", pady=(0, 5))
    
    # Frame para botones de zoom y nivel
    frame_botones_zoom = CTkFrame(frame_zoom, fg_color="transparent")
    frame_botones_zoom.pack(fill="x")
    
    # Botón de zoom -
    boton_zoom_menos = CTkButton(
        frame_botones_zoom,
        text="−",  # Signo menos Unicode
        command=lambda: ajustar_zoom(-0.1),
        width=25,
        height=25,
        corner_radius=8,
        fg_color=color_boton_principal,
    ...
    boton_zoom_menos.pack(side="left", padx=(0, 5))
    
    # Etiqueta que muestra el nivel actual de zoom
    global label_zoom_nivel
    label_zoom_nivel = CTkLabel(
        frame_botones_zoom, 
        text="Zoom: 100%",
        text_color=color_texto_claro,
        font=("Arial", 12),
    ...
    label_zoom_nivel.pack(side="left", fill="x", expand=True)
    
    # Botón de zoom +
    boton_zoom_mas = CTkButton(
        frame_botones_zoom,
        text="+",  
        command=lambda: ajustar_zoom(0.1),
        width=25,
    ...
    boton_zoom_mas.pack(side="right", padx=(5, 0))
    ...
    # Iniciar hilo de recepción - AGREGAR ESTA SECCIÓN
    ...
    ventana.protocol("WM_DELETE_WINDOW", lambda: salir(socket, username, ventana))

    ventana.mainloop()

def cerrar_aplicacion():
    """Cierra limpiamente la aplicación deteniendo todos los hilos activos"""
    # Detener todos los hilos de escaneo activos
    if hasattr(host_scanner, 'stop_event'):
        host_scanner.stop_event.set()
    if hasattr(port_scanner, 'stop_event'):
        port_scanner.stop_event.set()
    
    # Esperar brevemente a que los hilos terminen
    time.sleep(0.5)
    
    # Cerrar sockets si existen
