...
def analyze_banner(ip, port, service_name):
    ...
    try:
        ...
        return "Conexión rechazada por el host"
...
def scan_port_tcp(ip, port, timeout=1, stealth=False, delay=0):
    ...
    conf.verb = 0
    ip_layer = IP(dst=ip)
    tcp_layer = TCP(dport=port, flags="S", window=1024)  # Ajustar tamaño de ventana TCP
    ...
    try:
        ...
        return port, "Error", detect_service(port)
...
def scan_ports(ip, ports, timeout=1, stealth=False, delay=0, show_only_open=False):
    ...
    def scan(port):
        ...
        # Si se pide solo mostrar abiertos, filtrar el resto
...
def user_interface():
    ...
    try:
        speed = int(input("Selecciona el nivel de velocidad (0-5): ").strip())
        if speed not in range(6):
            raise ValueError
    except ValueError:
        print("Nivel de velocidad inválido. Saliendo...")
        return
    ...
    results = scan_ports(ip, ports, timeout=timeout, stealth=stealth,
