...
def user_interface():
    ...
    # Opción para el nivel de temporización
    print("\nSelecciona el nivel de temporización (similar a nmap -T):")
    print("0: Paranoid - Muy lento, ideal para evadir IDS")
    print("1: Sneaky - Lento y sigiloso")
    print("2: Polite - Moderado, menor carga en la red")
    print("3: Normal - Equilibrio entre velocidad y precisión (por defecto)")
    print("4: Aggressive - Rápido, asume buena conectividad")
    print("5: Insane - Muy rápido, puede perder información")
    
    timing_level = input("Nivel de temporización (0-5) [3]: ")
    if not timing_level or timing_level not in "012345":
        timing_level = 3
    else:
        timing_level = int(timing_level)
    ...
    print(f"\nIniciando escaneo de puertos con nivel de temporización T{timing_level}...")
    start_time = time.time()

    results = scan_ports(ip, ports, stealth=stealth, show_only_open=show_only_open, timing_level=timing_level)
    os_detected = os_fingerprinting(ip, TIMING_TEMPLATES[timing_level]["timeout"])
